jQuery(document).ready(function($) {
    jQuery(document).ready(function($) {
        function fetchFilteredPhotos() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('#photo-gallery').html(response); // Replace existing content with first page
                },
        ...
        // Function to toggle visibility of reset option text
        function toggleResetTextVisibility($dropdown, show) {
            var $resetOption = $dropdown.find('.dropdown-content li:first-child');
    
            if (show) {
                $resetOption.removeClass('hidden-label');
            } else {
                $resetOption.addClass('hidden-label');
            }
        }
